...
usage: script.py  lig_name   targ_name   path_to_xtal_lig   path_to_recp

./AD4_automation_v1.12.py adrb1 braf ./shared/crystal_ligand_braf.mol2 ./shared/recp_braf.pdbqt
'''
...
def write_AD4_config(mol_lig, pdbqt_lig, u_lst):
    ...
    try:
        ...
        conf1 = m.GetConformer()
        centroid = Chem.rdMolTransforms.ComputeCentroid(conf1)
        x_center = float(str(centroid.x))
        y_center = float(str(centroid.y))
        z_center = float(str(centroid.z))
